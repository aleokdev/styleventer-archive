...
# Function that gets a string from the rpgLang file found in languagePath defined in modutil.
# File must not include extension!
def getl(server, file, key):
    # Open rpgLang file found at languagePath
    f = open(
        os.path.join(modutil.absolutePath,
                     "lang", ("common" if server is None else modutil.getServerLanguage(server)),
                     # If no server is given, use "common" folder instead
                     file + ".txt"),
        "r",
        encoding="utf-8"
    )
